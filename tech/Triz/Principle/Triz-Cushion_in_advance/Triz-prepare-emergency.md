---
aliases:
- prepare-emergency
Confidential: public
copyright: "https://github.com/SpocWeb"
IsDeleted: false
IsReadOnly: false
license: "CC BY-SA 4.0"
tags:
- Triz/Principle/Sub
type: TrizPrincipleSub
---

[name::prepare-emergency]
[SpocWebEntityId::22260]
[TrizPrinciple::[[Triz-Cushion_in_advance]]]

Prepare emergency means beforehand to compensate for the relatively low reliability of an object.

## Examples: 

- #has_/examples :: Alternate Air System for Aircraft Instruments 
- #has_/examples :: Back-up Parachute
- #has_/examples :: Magnetized Merchandise to prevent Shop Lifting
- #has_/examples :: Magnetized Tools to hold Screws
- #has_/examples :: Magnetic Strip on photographic Film that directs the Developer to compensate for poor Exposure
- #has_/examples :: 
- 


## Confidential Links & Embeds: 
- [[../../../../_Standards/tech/Triz/Sub/Triz-prepare-emergency|Triz-prepare-emergency]] 
- [[../../../../_public/tech/Triz/Sub/Triz-prepare-emergency.public|Triz-prepare-emergency.public]] 
- [[../../../../_internal/tech/Triz/Sub/Triz-prepare-emergency.internal|Triz-prepare-emergency.internal]] 
- [[../../../../_protect/tech/Triz/Sub/Triz-prepare-emergency.protect|Triz-prepare-emergency.protect]] 
- [[../../../../_private/tech/Triz/Sub/Triz-prepare-emergency.private|Triz-prepare-emergency.private]] 
- [[../../../../_personal/tech/Triz/Sub/Triz-prepare-emergency.personal|Triz-prepare-emergency.personal]] 
- [[../../../../_secret/tech/Triz/Sub/Triz-prepare-emergency.secret|Triz-prepare-emergency.secret]] 
